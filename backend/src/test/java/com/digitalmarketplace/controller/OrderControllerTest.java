...
class OrderControllerTest {
    ...
    void checkoutReturnsCreatedWithItems() throws Exception {
        ...
        mockMvc.perform(post("/api/orders/checkout").header("X-User-Id", "2"))
    ...
    @Test
    void checkoutWithoutUserHeaderReturnsBadRequest() throws Exception {
    ...
    void getOrderReturnsDetail() throws Exception {
        ...
        mockMvc.perform(get("/api/orders/7").header("X-User-Id", "2"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(7))
    ...
    void getOrderWhenNotFoundReturnsNotFound() throws Exception {
        ...
        mockMvc.perform(get("/api/orders/99").header("X-User-Id", "2"))
                .andExpect(status().isNotFound());
    }

    @Test
    void getOrderWithInvalidIdReturnsBadRequest() throws Exception {
        mockMvc.perform(get("/api/orders/0").header("X-User-Id", "2"))
                .andExpect(status().isBadRequest());
    }
}
